...
if __name__ == "__main__":
    ...
    # Positional argument #1: attack type
    attacks = [f.name for f in os.scandir(script_dir) if f.is_dir()]
    help = "Type of attack performed"
    parser.add_argument("attack", type=str, choices=attacks, help=help)
    # Positional argument #2: experimental scenario
    exps = ["no-attack", "with-attack"]
    help = "Experimental scenario"
    parser.add_argument("scenario", type=str, choices=exps, help=help)
    # Parse arguments
    args = parser.parse_args()

    # Get working directory for the given attack and scenario
    scenario_dir = os.path.join(script_dir, args.attack, args.scenario)
    ...
    result_file_name = f"{device}_{args.attack}_{args.scenario}.csv"
