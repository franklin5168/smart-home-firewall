...
"""
Produce ground truth data for firewall evaluation,
by replaying the baseline PCAPs through the firewall
and recording each packet's verdict.
"""
...
def exit_cleanup(sig, frame):
    ...
    :param frame: (Unused) current stack frame
    """
...
if __name__ == "__main__":
    ...
    exit_code = run_cmd(f"ping -w 5 -I {interface} {ip_addr}")
    if exit_code != 0:
        logging.error(f"Cannot ping target at {ip_addr} from interface {interface}.")
        exit()
    ...
    for device, pcaps in device_pcaps.items():
        ...
        for pcap in pcaps:
            ...
            if not os.path.exists(pcap_path):
                logging.warning(f"PCAP not found: {pcap_path}")
                continue
            ...
            nfq_csv_file = f"{Path(pcap).stem}.nfq.csv"
            ...
            # Replay PCAP
            cmd = f"sudo tcpreplay-edit -T nano -i {interface} --enet-dmac={args.mac_addr} {pcap_path}"
            run_cmd(cmd)
