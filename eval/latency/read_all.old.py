...
def read_packets_from_pcap(pcap_name: str, pcap_path: str, device_timestamps: set, maps_addr_pcap: dict) -> Tuple[list, list]:
    ...
    for packet in raw_packets:
        ...
        device = get_device(timestamp, device_timestamps)
...
def get_device(timestamp: float, device_timestamps: dict) -> str:
    """
    Get the device related to the packet at the given timestamp.

    :param timestamp: packet timestamp
    :param device_timestamps: dictionary mapping devices to set of timestamps
    :return: device name
    """
...
if __name__ == "__main__":
    ...
    scenario_dir = os.path.join(script_dir, "all", args.scenario)
    ...
    result_file_name = f"all_{args.scenario}.csv"
    result_file_path = os.path.join(scenario_dir, result_file_name)
    with open(result_file_path, "w") as f:
        fieldnames = ["hash", "base_pcap", "base_id", "base_timestamp", "other_pcap", "other_id", "other_timestamp",  "packet", "latency", "device", "protocol_category"]
