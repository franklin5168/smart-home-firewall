...
#endif /* LOG */
{
    ...
    if (
        (
        // Newly added: coming back from last policy in the loop
        interactions_data[0].current_state == 1 ||  // First time entering the loop
        (interactions_data[0].in_loop && interactions_data[0].current_state == 3)  // Coming back in loop from stun-bind
        )
        && (
        dns_message.header.qr == 0
        &&
