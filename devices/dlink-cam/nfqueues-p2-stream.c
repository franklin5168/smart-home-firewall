...
#endif /* LOG */
{
    ...
    if (
        ( 
        interactions_data[9].current_state == 3 ||
        ...
        (interactions_data[9].counters + 2)->packet_count.value = 0;
        #ifdef DEBUG
        printf("Reset packet-count counter for interaction 9 and policy 2\n");
        #endif
        // Policy is one-off, increment state
        #ifdef DEBUG
    ...
    if (
        ...
        // Previous policy was transient
        // Reset packet counter
        (interactions_data[10].counters + 2)->packet_count.value = 0;
        #ifdef DEBUG
        printf("Reset packet-count counter for interaction 10 and policy 2\n");
        #endif
        // Policy is one-off, increment state
        #ifdef DEBUG
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-local#dns-cam-server-aaaa: State %hhu -> State 1\n", interactions_data[15].current_state);
        #endif
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-local#dns-cam-server-a: State %hhu -> State 3\n", interactions_data[15].current_state);
        #endif
    ...
    if (
        ...
        // Previous policy was transient
        // Reset packet counter
        (interactions_data[16].counters + 4)->packet_count.value = 0;
        #ifdef DEBUG
        printf("Reset packet-count counter for interaction 16 and policy 4\n");
        #endif
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("bootstrap-stream-wan#dns-cam-sas-1-aaaa: State %hhu -> State 1\n", interactions_data[16].current_state);
        #endif
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("bootstrap-stream-wan#dns-cam-sas-1-a: State %hhu -> State 3\n", interactions_data[16].current_state);
        #endif
    ...
    if (
        ( 
        (
        interactions_data[17].in_loop && 
        interactions_data[17].current_state == 6
        ) ||
        interactions_data[17].current_state == 6 ||
        ...
        interactions_data[17].in_loop = true;

        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-external#dns-cam-server-aaaa: State %hhu -> State 1\n", interactions_data[17].current_state);
        #endif
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-external#dns-cam-server-a: State %hhu -> State 3\n", interactions_data[17].current_state);
        #endif
        interactions_data[17].current_state = 3;
...
#endif /* LOG */
{
    ...
    if (
        ...
        #endif
        interactions_data[9].current_state = 2;

        if (ip_list.ip_count > 0) {
            // Add IP addresses to DNS map
            dns_map_add(dns_map, domain_name, ip_list);
            // Cache first IP address for quicker access
            interactions_data[9].cached_ip = *(ip_list.ip_addresses);
        }
    ...
    if (
        ...
        if (ip_list.ip_count > 0) {
            ...
            dns_map_add(dns_map, domain_name, ip_list);
            // Cache first IP address for quicker access
            interactions_data[11].cached_ip = *(ip_list.ip_addresses);
        }
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-local#dns-cam-server-aaaa-backward: State %hhu -> State 2\n", interactions_data[15].current_state);
        #endif
        interactions_data[15].current_state = 2;
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-local#dns-cam-server-a-backward: State %hhu -> State 4\n", interactions_data[15].current_state);
        #endif
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("bootstrap-stream-wan#dns-cam-sas-1-aaaa-backward: State %hhu -> State 2\n", interactions_data[16].current_state);
        #endif
        interactions_data[16].current_state = 2;
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("bootstrap-stream-wan#dns-cam-sas-1-a-backward: State %hhu -> State 4\n", interactions_data[16].current_state);
        #endif
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-external#dns-cam-server-aaaa-backward: State %hhu -> State 2\n", interactions_data[17].current_state);
        #endif
        interactions_data[17].current_state = 2;
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("stream-app-external#dns-cam-server-a-backward: State %hhu -> State 4\n", interactions_data[17].current_state);
        #endif
        interactions_data[17].current_state = 4;
...
/**
 * @brief single#mdns-query-ptr callback function, called when a packet enters the queue.
 * 
 * @param pkt_id packet ID for netfilter queue
 * @param hash packet payload SHA256 hash (only present if LOG is defined)
 * @param timestamp packet timestamp (only present if LOG is defined)
...
#ifdef LOG
uint32_t callback_single_mdns_query_ptr(int pkt_id, uint8_t *hash, struct timeval timestamp, int pkt_len, uint8_t *payload, void *arg)
#else
uint32_t callback_single_mdns_query_ptr(int pkt_id, int pkt_len, uint8_t *payload, void *arg)
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 20\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (verdict != NF_STOP) {
        // Log packet as dropped
        print_hash(hash);
        printf(",%ld.%06ld,single#mdns-query-ptr,,DROP\n", (long int)timestamp.tv_sec, (long int)timestamp.tv_usec);
    }
    free(hash);
    #endif /* LOG */
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 30\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-gateway-cam#arp-request-gateway-cam: State %hhu -> State 1\n", interactions_data[6].current_state);
        #endif
        interactions_data[6].current_state = 1;
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 31\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-gateway-cam#arp-reply-cam-gateway: State %hhu -> State 0\n", interactions_data[6].current_state);
        #endif
        interactions_data[6].current_state = 0;
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 40\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-cam-gateway#arp-request-cam-gateway: State %hhu -> State 1\n", interactions_data[7].current_state);
        #endif
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 41\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-cam-gateway#arp-reply-gateway-cam: State %hhu -> State 0\n", interactions_data[7].current_state);
        #endif
        interactions_data[7].current_state = 0;
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 50\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("get-ip-address#dhcp-discover: State %hhu -> State 1\n", interactions_data[8].current_state);
        #endif
    ...
    if (
        ( 
        interactions_data[8].current_state == 2
        )
        && (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("get-ip-address#dhcp-request: State %hhu -> State 3\n", interactions_data[8].current_state);
        #endif
        interactions_data[8].current_state = 3;
...
/**
 * @brief get-ip-address#dhcp-offer callback function, called when a packet enters the queue.
 * 
 * @param pkt_id packet ID for netfilter queue
 * @param hash packet payload SHA256 hash (only present if LOG is defined)
 * @param timestamp packet timestamp (only present if LOG is defined)
...
#ifdef LOG
uint32_t callback_get_ip_address_dhcp_offer(int pkt_id, uint8_t *hash, struct timeval timestamp, int pkt_len, uint8_t *payload, void *arg)
#else
uint32_t callback_get_ip_address_dhcp_offer(int pkt_id, int pkt_len, uint8_t *payload, void *arg)
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 51\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("get-ip-address#dhcp-offer: State %hhu -> State 2\n", interactions_data[8].current_state);
        #endif
        interactions_data[8].current_state = 2;
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("get-ip-address#dhcp-ack: State %hhu -> State 0\n", interactions_data[8].current_state);
        #endif
        interactions_data[8].current_state = 0;
    ...
    if (verdict != NF_STOP) {
        // Log packet as dropped
        print_hash(hash);
        printf(",%ld.%06ld,get-ip-address#dhcp-offer,,DROP\n", (long int)timestamp.tv_sec, (long int)timestamp.tv_usec);
    }
    free(hash);
    #endif /* LOG */
...
/**
 * @brief http-head-boot#http-head-cam-cloud callback function, called when a packet enters the queue.
 * 
 * @param pkt_id packet ID for netfilter queue
 * @param hash packet payload SHA256 hash (only present if LOG is defined)
 * @param timestamp packet timestamp (only present if LOG is defined)
...
#ifdef LOG
uint32_t callback_http_head_boot_http_head_cam_cloud(int pkt_id, uint8_t *hash, struct timeval timestamp, int pkt_len, uint8_t *payload, void *arg)
#else
uint32_t callback_http_head_boot_http_head_cam_cloud(int pkt_id, int pkt_len, uint8_t *payload, void *arg)
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 60\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (!(interactions_data[9].counters + 2)->packet_count.is_initialized) {
        (interactions_data[9].counters + 2)->packet_count.value = 0;
        (interactions_data[9].counters + 2)->packet_count.is_initialized = true;
    }
    // Initialize packet count if not initialized yet
    if (!(interactions_data[16].counters + 4)->packet_count.is_initialized) {
        (interactions_data[16].counters + 4)->packet_count.value = 0;
        (interactions_data[16].counters + 4)->packet_count.is_initialized = true;
    }
    ...
    #endif
    if (
        (interactions_data[9].counters + 2)->packet_count.is_initialized &&
        ( 
        interactions_data[9].current_state == 2 ||
        interactions_data[9].current_state == 3
        )
        ...
        &&
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}, interactions_data[9].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "ca-mgr.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}) )
        )
        )
    ) {
        ...
        (interactions_data[9].counters + 2)->packet_count.value++;
        if (
            (interactions_data[9].counters + 2)->packet_count.value >= 15
        ) {
            ...
            #endif
            interactions_data[9].current_state = 0;
            (interactions_data[9].counters + 2)->packet_count.is_initialized = false;
        }
    ...
    if (
        ...
        interactions_data[12].current_state == 2        )
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}, interactions_data[12].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-sas-1.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}) )
        )
    ) {
    ...
    #endif
    if (
        (interactions_data[16].counters + 4)->packet_count.is_initialized &&
        ( 
        interactions_data[16].current_state == 4 ||
        interactions_data[16].current_state == 5
        )
        ...
        &&
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}, interactions_data[16].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-sas-1.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}) )
        )
        )
    ) {
        ...
        interactions_data[16].current_state = 5;
        // Increment packet count
        (interactions_data[16].counters + 4)->packet_count.value++;
        if (
            (interactions_data[16].counters + 4)->packet_count.value >= 15
        ) {
            // Policy is transient and counters have been reached
            // Increment state to indicate the transient policy is over
            #ifdef DEBUG
            ...
            #endif
            interactions_data[16].current_state = 0;
            (interactions_data[16].counters + 4)->packet_count.is_initialized = false;
        }
    ...
    if (verdict != NF_STOP) {
        // Log packet as dropped
        print_hash(hash);
        printf(",%ld.%06ld,http-head-boot#http-head-cam-cloud,,DROP\n", (long int)timestamp.tv_sec, (long int)timestamp.tv_usec);
    }
    free(hash);
    #endif /* LOG */
...
/**
 * @brief http-head-boot#http-head-cam-cloud-backward callback function, called when a packet enters the queue.
 * 
 * @param pkt_id packet ID for netfilter queue
 * @param hash packet payload SHA256 hash (only present if LOG is defined)
 * @param timestamp packet timestamp (only present if LOG is defined)
...
#ifdef LOG
uint32_t callback_http_head_boot_http_head_cam_cloud_backward(int pkt_id, uint8_t *hash, struct timeval timestamp, int pkt_len, uint8_t *payload, void *arg)
#else
uint32_t callback_http_head_boot_http_head_cam_cloud_backward(int pkt_id, int pkt_len, uint8_t *payload, void *arg)
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 61\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        !http_message.is_request
        &&
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = src_addr}, interactions_data[9].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "ca-mgr.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = src_addr}) )
        )
        )
    ) {
    ...
    if (
        ...
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = src_addr}, interactions_data[12].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-sas-1.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = src_addr}) )
        )
    ) {
    ...
    if (
        ...
        !http_message.is_request
        &&
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = src_addr}, interactions_data[16].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-sas-1.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = src_addr}) )
        )
        )
    ) {
    ...
    if (verdict != NF_STOP) {
        // Log packet as dropped
        print_hash(hash);
        printf(",%ld.%06ld,http-head-boot#http-head-cam-cloud-backward,,DROP\n", (long int)timestamp.tv_sec, (long int)timestamp.tv_usec);
    }
    free(hash);
    #endif /* LOG */
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 70\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    // Initialize packet count if not initialized yet
    if (!(interactions_data[10].counters + 2)->packet_count.is_initialized) {
        (interactions_data[10].counters + 2)->packet_count.value = 0;
        (interactions_data[10].counters + 2)->packet_count.is_initialized = true;
    }
    ...
    #endif
    if (
        (interactions_data[10].counters + 2)->packet_count.is_initialized &&
        ( 
        interactions_data[10].current_state == 2 ||
        interactions_data[10].current_state == 3
        )
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}, interactions_data[10].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-fwd.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}) )
        )
    ) {
        ...
        interactions_data[10].current_state = 3;
        // Increment packet count
        (interactions_data[10].counters + 2)->packet_count.value++;
        if (
            (interactions_data[10].counters + 2)->packet_count.value >= 30
        ) {
            // Policy is transient and counters have been reached
            // Increment state to indicate the transient policy is over
            #ifdef DEBUG
            ...
            #endif
            interactions_data[10].current_state = 0;
            (interactions_data[10].counters + 2)->packet_count.is_initialized = false;
        }
    ...
    if (
        ...
        interactions_data[11].current_state == 2        )
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}, interactions_data[11].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-dcdda.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}) )
        )
    ) {
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 71\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = src_addr}, interactions_data[10].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-fwd.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = src_addr}) )
        )
    ) {
    ...
    if (
        ...
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = src_addr}, interactions_data[11].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-dcdda.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = src_addr}) )
        )
    ) {
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 80\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-camera-phone#arp-request-cam-phone: State %hhu -> State 1\n", interactions_data[13].current_state);
        #endif
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 81\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-camera-phone#arp-reply-phone-cam: State %hhu -> State 0\n", interactions_data[13].current_state);
        #endif
        interactions_data[13].current_state = 0;
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 90\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-phone-camera#arp-request-phone-cam: State %hhu -> State 1\n", interactions_data[14].current_state);
        #endif
        interactions_data[14].current_state = 1;
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 91\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        // Policy is one-off, increment state
        #ifdef DEBUG
        printf("arp-phone-camera#arp-reply-cam-phone: State %hhu -> State 0\n", interactions_data[14].current_state);
        #endif
        interactions_data[14].current_state = 0;
...
/**
 * @brief stream-app-local#stun-bind-cam-server callback function, called when a packet enters the queue.
 * 
 * @param pkt_id packet ID for netfilter queue
 * @param hash packet payload SHA256 hash (only present if LOG is defined)
 * @param timestamp packet timestamp (only present if LOG is defined)
...
#ifdef LOG
uint32_t callback_stream_app_local_stun_bind_cam_server(int pkt_id, uint8_t *hash, struct timeval timestamp, int pkt_len, uint8_t *payload, void *arg)
#else
uint32_t callback_stream_app_local_stun_bind_cam_server(int pkt_id, int pkt_len, uint8_t *payload, void *arg)
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 100\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}, interactions_data[17].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-sas.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = dst_addr}) )
        )
    ) {
        ...
        #endif
        interactions_data[17].current_state = 5;
    ...
    if (verdict != NF_STOP) {
        // Log packet as dropped
        print_hash(hash);
        printf(",%ld.%06ld,stream-app-local#stun-bind-cam-server,,DROP\n", (long int)timestamp.tv_sec, (long int)timestamp.tv_usec);
    }
    free(hash);
    #endif /* LOG */
...
/**
 * @brief stream-app-local#stun-bind-cam-server-backward callback function, called when a packet enters the queue.
 * 
 * @param pkt_id packet ID for netfilter queue
 * @param hash packet payload SHA256 hash (only present if LOG is defined)
 * @param timestamp packet timestamp (only present if LOG is defined)
...
#ifdef LOG
uint32_t callback_stream_app_local_stun_bind_cam_server_backward(int pkt_id, uint8_t *hash, struct timeval timestamp, int pkt_len, uint8_t *payload, void *arg)
#else
uint32_t callback_stream_app_local_stun_bind_cam_server_backward(int pkt_id, int pkt_len, uint8_t *payload, void *arg)
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 101\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        && (
        ( compare_ip((ip_addr_t) {.version = 4, .value.ipv4 = src_addr}, interactions_data[17].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "mp-eu-sas.auto.mydlink.com"), (ip_addr_t) {.version = 4, .value.ipv4 = src_addr}) )
        )
    ) {
    ...
    if (verdict != NF_STOP) {
        // Log packet as dropped
        print_hash(hash);
        printf(",%ld.%06ld,stream-app-local#stun-bind-cam-server-backward,,DROP\n", (long int)timestamp.tv_sec, (long int)timestamp.tv_usec);
    }
    free(hash);
    #endif /* LOG */
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 102\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 103\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 110\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
    ...
    if (
        ...
        if (interactions_data[17].in_loop) {
...
#endif /* LOG */
{
    #if defined DEBUG || defined LOG
    #ifdef DEBUG
    printf("Received packet from nfqueue 111\n");
    #endif
    uint8_t previous_state;
    #endif /* DEBUG || LOG */
...
int main(int argc, char const *argv[]) {
    ...
    interactions_data[0].nfq_id_base = 0;
    ...
    interactions_data[1].nfq_id_base = 10;
    ...
    interactions_data[2].nfq_id_base = 20;
    ...
    interactions_data[3].nfq_id_base = 30;
    ...
    interactions_data[4].nfq_id_base = 30;
    ...
    interactions_data[5].nfq_id_base = 30;
    ...
    interactions_data[6].nfq_id_base = 30;
    ...
    interactions_data[7].nfq_id_base = 40;
    ...
    interactions_data[8].nfq_id_base = 50;
    interactions_data[8].num_policies = 4;
    interactions_data[8].num_states = 4;
    ret = pthread_mutex_init(&(interactions_data[8].mutex), NULL);
    assert(ret == 0);
    interactions_data[8].current_state = 0;
    interactions_data[8].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[8].counters = (counters_t *) malloc(sizeof(counters_t) * 4);
    ...
    interactions_data[9].nfq_id_base = 60;
    interactions_data[9].num_policies = 4;
    interactions_data[9].num_states = 4;
    ret = pthread_mutex_init(&(interactions_data[9].mutex), NULL);
    assert(ret == 0);
    interactions_data[9].current_state = 0;
    interactions_data[9].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[9].counters = (counters_t *) malloc(sizeof(counters_t) * 4);
    ...
    interactions_data[10].nfq_id_base = 70;
    interactions_data[10].num_policies = 4;
    interactions_data[10].num_states = 4;
    ret = pthread_mutex_init(&(interactions_data[10].mutex), NULL);
    assert(ret == 0);
    interactions_data[10].current_state = 0;
    interactions_data[10].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[10].counters = (counters_t *) malloc(sizeof(counters_t) * 4);
    ...
    interactions_data[11].nfq_id_base = 80;
    interactions_data[11].num_policies = 4;
    interactions_data[11].num_states = 3;
    ...
    interactions_data[12].nfq_id_base = 80;
    interactions_data[12].num_policies = 4;
    interactions_data[12].num_states = 3;
    ...
    interactions_data[13].nfq_id_base = 80;
    interactions_data[13].num_policies = 2;
    interactions_data[13].num_states = 2;
    ret = pthread_mutex_init(&(interactions_data[13].mutex), NULL);
    assert(ret == 0);
    interactions_data[13].current_state = 0;
    interactions_data[13].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[13].counters = (counters_t *) malloc(sizeof(counters_t) * 2);
    ...
    interactions_data[14].nfq_id_base = 90;
    interactions_data[14].num_policies = 2;
    interactions_data[14].num_states = 2;
    ret = pthread_mutex_init(&(interactions_data[14].mutex), NULL);
    assert(ret == 0);
    interactions_data[14].current_state = 0;
    interactions_data[14].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[14].counters = (counters_t *) malloc(sizeof(counters_t) * 2);
    ...
    interactions_data[15].nfq_id_base = 100;
    ...
    interactions_data[16].nfq_id_base = 110;
    interactions_data[16].num_policies = 6;
    interactions_data[16].num_states = 6;
    ret = pthread_mutex_init(&(interactions_data[16].mutex), NULL);
    assert(ret == 0);
    interactions_data[16].current_state = 0;
    interactions_data[16].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[16].counters = (counters_t *) malloc(sizeof(counters_t) * 6);
    ...
    interactions_data[17].nfq_id_base = 110;
    interactions_data[17].num_policies = 8;
    interactions_data[17].num_states = 7;
    ret = pthread_mutex_init(&(interactions_data[17].mutex), NULL);
    assert(ret == 0);
    interactions_data[17].current_state = 0;
    interactions_data[17].cached_ip = (ip_addr_t) {.version = 0, .value.ipv4 = 0};
    // Allocate memory for counters
    interactions_data[17].counters = (counters_t *) malloc(sizeof(counters_t) * 8);
    ...
    // single_mdns_query_ptr
    thread_arg_t thread_arg_single_mdns_query_ptr = {
        .queue_id = 20,
        .func = &callback_single_mdns_query_ptr,
        .arg = NULL
    };
    ret = pthread_create(&threads[i++], NULL, nfqueue_thread, (void *) &thread_arg_single_mdns_query_ptr);
    assert(ret == 0);
    ...
    // get_ip_address_dhcp_offer
    thread_arg_t thread_arg_get_ip_address_dhcp_offer = {
        .queue_id = 51,
        .func = &callback_get_ip_address_dhcp_offer,
        .arg = NULL
    };
    ret = pthread_create(&threads[i++], NULL, nfqueue_thread, (void *) &thread_arg_get_ip_address_dhcp_offer);
    assert(ret == 0);
    
    // http_head_boot_http_head_cam_cloud
    thread_arg_t thread_arg_http_head_boot_http_head_cam_cloud = {
        .queue_id = 60,
        .func = &callback_http_head_boot_http_head_cam_cloud,
        .arg = NULL
    };
    ret = pthread_create(&threads[i++], NULL, nfqueue_thread, (void *) &thread_arg_http_head_boot_http_head_cam_cloud);
    assert(ret == 0);
    
    // http_head_boot_http_head_cam_cloud_backward
    thread_arg_t thread_arg_http_head_boot_http_head_cam_cloud_backward = {
        .queue_id = 61,
        .func = &callback_http_head_boot_http_head_cam_cloud_backward,
        .arg = NULL
    };
    ret = pthread_create(&threads[i++], NULL, nfqueue_thread, (void *) &thread_arg_http_head_boot_http_head_cam_cloud_backward);
    assert(ret == 0);
    ...
    // stream_app_local_stun_bind_cam_server
    thread_arg_t thread_arg_stream_app_local_stun_bind_cam_server = {
        .queue_id = 100,
        .func = &callback_stream_app_local_stun_bind_cam_server,
        .arg = NULL
    };
    ret = pthread_create(&threads[i++], NULL, nfqueue_thread, (void *) &thread_arg_stream_app_local_stun_bind_cam_server);
    assert(ret == 0);
    
    // stream_app_local_stun_bind_cam_server_backward
    thread_arg_t thread_arg_stream_app_local_stun_bind_cam_server_backward = {
        .queue_id = 101,
        .func = &callback_stream_app_local_stun_bind_cam_server_backward,
        .arg = NULL
    };
    ret = pthread_create(&threads[i++], NULL, nfqueue_thread, (void *) &thread_arg_stream_app_local_stun_bind_cam_server_backward);
    assert(ret == 0);
